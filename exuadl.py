...
def wget(options, exit_if_directory_exists=False, cwd=os.getcwd()):
    ...
    if not options.faststart or not options.urls_original:
        html_data = exualib.read_url_content(options)

        urls = exualib.parse_file_urls(html_data)
        # ansi.print_line("Found " + str(len(urls)) + " files. ")

        if not urls:
            dirs = exualib.parse_all_links_urls_paged(options.url)
            # ansi.print_line('Found %s dirs.' % len(dirs))

        if options.level >= 1:
            cwd_ = cwd + '/' + exualib.parse_folder_name(html_data)
            if options.level >= 2:
                cwd_ = cwd + '/' + exualib.parse_parent_folder_name(html_data) + '/' + cwd
...
def resolve_urls(urls):
    return list(map(exualib.get_real_url, urls))
